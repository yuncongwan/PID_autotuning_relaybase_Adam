...
'''
def Adam_finetune(tuning_rule,pid_pre_tune):
    grad = np.array([0.,0.,0.])
    adam = AdamOptimiser(pid_pre_tune,alpha=args.lr)
    for step in range(args.max_steps):
        for index in range(3):
            grad[index] = calc_grad(tuning_rule,pid_pre_tune,index)
        pid_pre_tune = adam.backward_pass(grad)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        if args.autotune:
            ...
            simulate_pid(args,Adam_finetune("ziegler-nichols",pid_pretune))
